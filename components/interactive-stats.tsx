...
const AnimatedCounter = ({ target, suffix = "", duration = 2000 }: AnimatedCounterProps) => {
  const [ref, isVisible] = useIntersectionObserver() as [React.RefObject<HTMLSpanElement>, boolean];
  const [count, setIsActive] = useAnimatedCounter(target, duration) as [number, React.Dispatch<React.SetStateAction<boolean>>];
